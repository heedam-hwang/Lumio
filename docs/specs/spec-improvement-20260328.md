...
- As-is
  ...
  - 책 옵션에서 표지를 변경할 수는 있지만, 기존 표지를 제거하는 액션은 제공되지 않습니다.
- To-be
  - more menu 버튼은 원형 강조 스타일 대신 plain 버튼 스타일을 사용합니다.
  - 버튼의 탭 영역이 충분히 유지되도록 내부 패딩을 명시적으로 적용합니다.
  - 책 옵션에는 `"표지 변경"`과 함께 `"표지 삭제"` 액션을 제공하고, 표지가 없을 때는 삭제 액션을 비활성화하거나 노출하지 않습니다.

### 2.2 Improve the Empty Cover UI on the Book List Screen
- As-is
  - 표지가 없는 책 카드에는 `"Lumio"` 텍스트와 컬러 바 장식이 표시되어, 실제 책 표지 플레이스홀더보다는 템플릿 느낌이 강합니다.
  - 플레이스홀더 배경이 고정되어 있어 카드 간 시각적 구분이 약합니다.
- To-be
  - 표지가 없을 때 `"Lumio"` 텍스트는 제거합니다.
  - 기존 컬러 바 장식은 제거합니다.
  - trailing bottom 위치에 책 아이콘을 배치해 표지 없음 상태를 직관적으로 보여줍니다.
  - 플레이스홀더 배경색은 책마다 랜덤하게 보이되, 같은 책에서는 일관되게 유지되도록 지정합니다.

### 2.3 Clean Up the Top Area of the Page List Screen
- As-is
...
### 2.4 Align Page List Edit Actions with the Page Detail Screen
- As-is
...
### 2.5 Simplify the Page Detail Screen Header Actions
- As-is
...
### 2.6 Improve the Vocabulary Screen Header and Button Interaction
- As-is
...
### 2.7 Defer Vocabulary Deletion Until the User Leaves the Screen
- As-is
...
- 표지 옵션에는 변경과 삭제가 모두 포함되어야 하며, 표지가 없는 상태의 카드도 시각적으로 완성도 있게 보여야 합니다.
...
- Test scope: 책 목록 more menu 버튼 탭 영역, 표지 변경 및 표지 삭제 동작, 표지 없는 책 카드 플레이스홀더 UI, 페이지 리스트 divider 및 시간 텍스트 표시, 페이지 리스트/상세 편집 버튼 문구 통일, 단어장 navigation title 및 버튼 독립 탭, 단어장 북마크 해제 후 화면 이탈 시 삭제 반영

## 5. Change Log
- `2026.03.28`: 책 목록, 페이지 리스트, 페이지 상세, 단어장 화면 개선 요구사항 추가
- `2026.03.28`: 책 표지 삭제 액션과 표지 없음 상태 UI 개선 요구사항 추가
